...
class SOM(object):
    ...
    def update_weights(self, time_step):
        ...
        # OLD: Update the weights according to slide L16-10
        ...
        for j in range(len(self.output_neurons)):
            # Compute deltas
            delta_w_jx = lr * self.topology_matrix[self.winner_index][j] * (self.input_neurons[sample_index].x - self.output_neurons[j].x)
            delta_w_jy = lr * self.topology_matrix[self.winner_index][j] * (self.input_neurons[sample_index].y - self.output_neurons[j].y)
