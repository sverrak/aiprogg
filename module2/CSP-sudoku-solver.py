...
class CSP:
    ...
    def backtrack(self, assignment):
        ...
        for value in assignment:
            if len(assignment[value]) == 1:
                length += 1
        if length == len(assignment):
            # print("Backtracking found a SOLUTION!")
            self.domains = copy.deepcopy(assignment)    # updating the domain to the current (and correct) assignment
            return assignment

        variable = self.select_unassigned_variable(assignment)
        for value in assignment[variable]:

            # Deep copy assignment to make sure no permanent changes are being made
            deep_copy = copy.deepcopy(assignment)

            # add variable = value to deep_copy and remove all the other values from the assignment
            deep_copy[variable] = value

            # PRINT only for debugging
            # print("Assignment:", deep_copy)
            # print("Variable:", variable)
            # print("Domain:", assignment[variable])
            # print("Value: ", value)
    ...
    def revise(self, assignment, i, j):
        ...
        for x in assignment[i]:
            ...
            if counter == len(assignment[j]):
                assignment[i].remove(x)     # eliminating for the assignment
                revised = True
        return revised
